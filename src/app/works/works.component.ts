...
export class WorksComponent implements OnInit{ 
  ...
  .subscribe((data:any[]) => { 
    next: { 
      let filteredProjects = data.filter((project: any) => project.stacks.includes("backend")); 
      this.displayedProjects = filteredProjects;
    } 
    error: console.log()
  })
  ...
  .subscribe((data:any[]) => { 
    next: { 
      let filteredProjects = data.filter((project: any) => project.stacks.includes("frontend")); 
      this.displayedProjects = filteredProjects;
    } 
    error: console.log()
  })
  ...
  verifyStack(stack : string) : boolean { 
    let isActive = this.displayedProjects.every((project : any) => project.stacks.includes(stack))
